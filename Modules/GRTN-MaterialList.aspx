<%@ Page Title="" EnableEventValidation="false" Language="C#" MasterPageFile="~/ModulesPage.Master" AutoEventWireup="true" CodeBehind="GRTN-MaterialList.aspx.cs" Inherits="FruitUnitedMobile.Modules.GRTN_MaterialList" %>
        ...
        <div class="container mt-4 parent" style="height: 100%;">
            <div class="form-container" style="padding-bottom: 80px; overflow-x: hidden;">
                ...
                <table style="width: 100%;">
                    ...
                    <tr>
                        ...
                        <td>
                            <div style="display: inline-block; width: 35%">
                                <asp:TextBox ID="QtyTB" CssClass="form-control" TextMode="Number" oninput="CalculateInternalQty();" Text="1" runat="server"></asp:TextBox>
                            </div>
                            <div style="display: inline-block; width: 55%; height: 100%">
                                <asp:DropDownList ID="ddlUOM" Width="100%" CssClass="form-control dropdown-mobile" onchange="SelectUOM_OnChange(this);" runat="server"></asp:DropDownList>
                                <asp:HiddenField ID="Select_UOM_Profile_ID_TB" runat="server"></asp:HiddenField>
                            </div>
                        </td>
                    </tr>
                    <tr>
                        <td class="TitleLabel">
                            <label class="label" for="QtyPerPackTB">Qty Per Pack</label>
                        </td>
                        <td>
                            <div style="display: inline-block; width: 35%">
                                <asp:TextBox ID="QtyPerPackTB" CssClass="form-control" TextMode="Number" oninput="CalculateInternalQty();" Text="1" runat="server"></asp:TextBox>
                            </div>
                    ...
                    <tr>
                        <td class="TitleLabel">
                            <label class="label" for="InternalQtyTB">Interal Qty</label>
                        </td>
                        <td>
                            <div style="display: inline-block; width: 35%">
                                <asp:TextBox ID="InternalQtyTB" CssClass="form-control" Text="1" runat="server" ReadOnly="true"></asp:TextBox>
                            </div>
                            <div style="display: inline-block; width: 35%; height: 100%">
                            ...
                            <Columns>
                                ...
                                <asp:TemplateField HeaderText="Project">
                                    <ItemTemplate>
                                        ...
                                        <br>
                                        <asp:Label ID="Label3" Text='Qty : ' Font-Size="Small" runat="server"></asp:Label>
                                        <asp:Label ID="Label4" Text='<%# Bind("Qty") %>' Font-Size="Small" runat="server"></asp:Label>
                                        <asp:Label ID="Label5" Text='<%# Bind("Selected_UOM") %>' Font-Size="Small" runat="server"></asp:Label>
                                        <br>
                                        <asp:Label ID="Label6" Text='Qty Per Pack : ' Font-Size="Small" runat="server"></asp:Label>
                                        <asp:Label ID="Label7" Text='<%# Bind("Qty_Per_Pack") %>' Font-Size="Small" runat="server"></asp:Label>
                                        <br>
                                        <asp:Label ID="Label8" Text='<%# Bind("Serial_No") %>' Font-Size="Small" runat="server"></asp:Label>
                                        <asp:HiddenField ID="Inventory_Ledger_ID_HF" Value='<%# Bind("Inventory_Ledger_ID") %>' runat="server"></asp:HiddenField>
                                    </ItemTemplate>
                                </asp:TemplateField>
                                <asp:TemplateField HeaderText="Qty">
                                    <ItemTemplate>
                                        <asp:Label ID="QtyTxt" Text='<%# Convert.ToInt32(Eval("Qty")) * Convert.ToInt32(Eval("Qty_Per_Pack")) %>' Font-Size="Small" runat="server"></asp:Label>
                                        <br>
                                        <asp:Label ID="Label9" Text='<%# Bind("UOM") %>' Font-Size="Small" runat="server"></asp:Label>
                                    </ItemTemplate>
                                </asp:TemplateField>
    ...
    <script>
        $(document).ready(function () {
            ...
            $("#<%= btnAdd.ClientID %>").click(function (e) {
                ...
                var val4 = $("#<%= ddlUOM.ClientID %>").val();
                var val5 = $("#<%= QtyTB.ClientID %>").val();
                var val6 = $("#<%= QtyPerPackTB.ClientID %>").val();
                ...
                if (val5.trim() === "" || !/^\d+$/.test(val5) || val5 == "0") {
                    alert("QTY cannot be empty or 0 and must be numbers only!");
                    e.preventDefault();
                    return false;
                }

                if (val6.trim() === "" || !/^\d+$/.test(val6) || val6 == "0") {
                    alert("QTY Per Pack cannot be empty or 0 and must be numbers only!");
                    e.preventDefault();
                    return false;
                }
            });
        ...
        function LoadddlPart(BrandDropDown) {
            ...
            var ddlUOM = $("#<%= ddlUOM.ClientID %>");
            ddlUOM.val('0').trigger('change');
            ddlUOM.off('.select2block');
            ddlUOM.next('.select2-container').removeClass('select2-disabled-style');

            if (BrandID == "0") {
                $("#<%= ddlPart.ClientID %>").empty();
                $("#<%= ddlPart.ClientID %>").append('<option value="0">-- Please Select --</option>');
                var QtyPerPack = document.getElementById('<%= QtyPerPackTB.ClientID %>');
                if (QtyPerPack.hasAttribute('readonly')) {
                    QtyPerPack.removeAttribute('readonly');
                }
                return;
            }
            ...
            $.ajax({
                type: "POST",
                url: "GRTN-MaterialList.aspx/GetddlPart",
        ...
        function LoadddlSerialNo(PartDropDown) {
            ...
            if (PartID == "0") {
                ...
                $.ajax({
                    type: "POST",
                    url: "GRTN-MaterialList.aspx/GetddlSerialNo",
                    ...
                    success: function (response) {
                        ...
                        $.each(Material_SerialNo, function (i, Serial_No) {
                            ddlSN.append('<option value="' + Serial_No.Inventory_Ledger_ID + '">' + Serial_No.Serial_No.toString() + '</option>');
                        });
                    },
                    error: function (xhr, status, error) {
                        console.error(xhr.responseText);
        ...
        function LoadUOMFromPart(PartDropDown) {
            ...
            if (PartID === "0") {
                ...
                ddlUOM.off('.select2block');
                ddlUOM.next('.select2-container').removeClass('select2-disabled-style');
                $("#<%= Select_UOM_Profile_ID_TB.ClientID %>").val('0');
                ...
                var QtyPerPack = document.getElementById('<%= QtyPerPackTB.ClientID %>');
                QtyPerPack.setAttribute('readonly', true);

                var dropdown = $('#<%= ddlUOM.ClientID %>');
                dropdown.on('select2:opening.select2block select2:closing.select2block', function (e) {
                    e.preventDefault();
                });
                dropdown.next('.select2-container').addClass('select2-disabled-style');

                $.ajax({
                    type: "POST",
                    url: "GRTN-MaterialList.aspx/GetUOMFromPart",
                    data: JSON.stringify({ PartID: PartID }),
                    contentType: "application/json; charset=utf-8",
                    dataType: "json",
                    success: function (response) {
                        ...
                        $.each(UOM, function (i, UOM_Text) {
                            if (UOM_Text.Loose_Packaging === 'Y') {
                                dropdown.off('.select2block');
                                dropdown.next('.select2-container').removeClass('select2-disabled-style');
                                QtyPerPack.removeAttribute('readonly');
                            }
                            if (UOM_Text.Sequence === '1') {
                                var optionValue = dropdown.find('option:contains("' + UOM_Text.UOM + '")').val();
        ...
        function SerialNo_OnChange(SerialNoDropDown) {
            ...
            if (SerialNo_ID == "0") {
                ...
                $('#<%= QtyPerPackTB.ClientID %>').val('1');
                $('#<%= QtyPerPackTB.ClientID %>').prop("readonly", false);
                $('#<%= InternalQtyTB.ClientID %>').val('1');
                ...
                $('#<%= QtyPerPackTB.ClientID %>').val('1');
                $('#<%= QtyPerPackTB.ClientID %>').prop("readonly", true);
                $('#<%= InternalQtyTB.ClientID %>').val('1');
        ...
        function SelectUOM_OnChange(UOMDropDown) {
            var UOMID = UOMDropDown.value;
            if (UOMID == "0") {
                $("#<%= Select_UOM_Profile_ID_TB.ClientID %>").val('0');
                return;
            }
            else {
                $("#<%= Select_UOM_Profile_ID_TB.ClientID %>").val(UOMID);
                return;
            }
        }

        function CalculateInternalQty() {
            try {
                var QtyTB = document.getElementById("ContentPlaceHolder1_QtyTB");
                var QtyPerPackTB = document.getElementById("ContentPlaceHolder1_QtyPerPackTB");
                var InternalQtyTB = document.getElementById("ContentPlaceHolder1_InternalQtyTB");
                InternalQtyTB.value = parseFloat(QtyTB.value) * parseFloat(QtyPerPackTB.value)

                if (InternalQtyTB.value === 'NaN') {
                    $('#<%= InternalQtyTB.ClientID %>').val('0');
                }
            }
            catch
            {
